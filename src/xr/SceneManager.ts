/**
 * SceneManager — Three.js сцена + стерео/моно переключение
 */
...
export class SceneManager {
  ...
  constructor(container: HTMLElement) {
    ...
    // Ограничиваем DPR для производительности на мобильных
    ...
    const pt = new THREE.PointLight(0x6366f1, 2, 10)
    pt.position.set(0, 2, 1)
    this.scene.add(pt)

    window.addEventListener('resize',            () => this.onResize())
    // Реагируем на поворот телефона
    window.addEventListener('orientationchange', () => setTimeout(() => this.onResize(), 300))
    screen.orientation?.addEventListener?.('change', () => setTimeout(() => this.onResize(), 300))
  }
  ...
  setupARBackground(video: HTMLVideoElement): void {
    ...
    this.bgScene.add(new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({
        map: this.videoTexture, depthTest: false, depthWrite: false
      })
    ))

    if (this.stereoRenderer && this.videoTexture) {
