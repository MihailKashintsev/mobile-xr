...
async function main(): Promise<void> {
  ...
  // Тасктбар: квадратные иконки — разделяем emoji и текст пробелом
  taskbar.setButtons([
  ...
  function animate(): void {
    ...
    // fingerWorld — для HandMesh (реальная глубина руки)
    const fingerWorld=[
...
/** Проецирует нормализованный landmark на реальную дистанцию в сцене */
function landmarkToWorld(lm:Landmark,cam:THREE.PerspectiveCamera,isFront:boolean):THREE.Vector3{
  ...
  const ndcY=-(lm.y*2-1)
  // Телефон держат ~50-75см от лица, lm.z ≈ 0 на этой дистанции
  // Небольшой Z offset для ощущения глубины руки
