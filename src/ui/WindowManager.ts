 ...
 *
 * ИЗМЕНЕНИЯ:
 * - Убраны три цветных шарика
 * - Кнопка закрытия: красивый ✕ в правом верхнем углу (3D mesh)
 * - Drag: кулак (grab), Press: три пальца (three_finger) hold 2 сек
 * - Hold-таймер без привязки к конкретной руке
...
function lighten(c: number, a: number): number {
  return (Math.min(255, ((c>>16)&255)+a) << 16)
       | (Math.min(255, ((c>> 8)&255)+a) <<  8)
       | (Math.min(255,  (c    &255)+a))
}

/** Кнопка с прогресс-кольцом */
...
function titleTex(t: string): THREE.CanvasTexture {
  return canvasTex((ctx, W, H) => {
    ...
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle'
    ctx.fillText(t, W/2, H/2, W-24)
  }, 1024, 96)
}

function closeBtnTex(): THREE.CanvasTexture {
  return canvasTex((ctx, W, H) => {
    // Фон — красноватый круг
...
export class XRWindow {
  ...
  private closeBtn?: THREE.Mesh   // 3D-кнопка закрытия
  btnEntries: BtnEntry[] = []
  private _dragging = false

  constructor(opts: WinOptions) {
    this.W = opts.width  ?? 1.5
    this.H = opts.height ?? 1.0
  ...
  private build(title: string, content: WinContent): void {
    ...
    const panel = new THREE.Mesh(
      ...
      new THREE.MeshPhysicalMaterial({
        color: C.bg, transparent: true, opacity: .93,
        roughness: .06, metalness: .12,
      })
    )
    ...
    const al = new THREE.Mesh(
      new THREE.BoxGeometry(W, .0025, PD + .006),
      new THREE.MeshBasicMaterial({ color: C.accent })
    )
    al.position.set(0, H/2 - TH, .002); this.group.add(al)

    // ── Кнопка закрытия — квадратная, правый верхний угол ────────────────────
    if (this.closeable) {
      const CSIZE = 0.072  // размер кнопки

      this.closeBtn = new THREE.Mesh(
        new THREE.BoxGeometry(CSIZE, CSIZE - 0.004, BTN_D + 0.006),
        new THREE.MeshPhysicalMaterial({
          map: closeBtnTex(),
          transparent: true, opacity: .96, roughness: .10,
        })
      )
      // Правый верхний угол, выступает вперёд
      this.closeBtn.position.set(W/2 - CSIZE/2 - 0.010, H/2 - TH/2, PD/2 + BTN_D/2 + 0.008)
      this.group.add(this.closeBtn)
    }

    // Title text (сдвигаем влево чтобы не перекрывалось кнопкой закрытия)
    const titleW = this.closeable ? W * .72 : W * .82
    const tt = new THREE.Mesh(
      new THREE.PlaneGeometry(titleW, TH * .65),
      new THREE.MeshBasicMaterial({ map: titleTex(title), transparent: true, depthWrite: false })
    )
    tt.position.set(this.closeable ? -0.040 : 0, H/2 - TH/2, PD/2 + .004)
    this.group.add(tt)
    ...
    for (let row = 0; row < 3; row++) {
      const gl = new THREE.Mesh(
        new THREE.BoxGeometry(W * .30, .0018, PD + .006),
        new THREE.MeshBasicMaterial({ color: 0x243350, transparent: true, opacity: .65 })
      )
      gl.position.set(0, -H/2 + BRH/2 + row * .016 - .016, .001)
      this.group.add(gl)
    }
    ...
    this.glow = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({
        color: C.accent, transparent: true, opacity: 0,
  ...
  hitButtonByFinger(fw: THREE.Vector3): BtnEntry | null {
    ...
    for (const e of this.btnEntries) {
      ...
      if (
        Math.abs(local.x) < p.width/2  + .07 &&
        Math.abs(local.y) < p.height/2 + .07 &&
        Math.abs(local.z) < p.depth/2  + .18
      ) return e
    }
    return null
  }
  ...
  hitButtonByRay(wp: THREE.Vector3): BtnEntry | null {
    ...
    for (const e of this.btnEntries) {
      ...
      const p = (e.mesh.geometry as THREE.BoxGeometry).parameters
      if (
        Math.abs(l.x) < p.width/2  + .04 &&
        Math.abs(l.y) < p.height/2 + .04 &&
        Math.abs(l.z) < .16
      ) return e
    }
    return null
  ...
  pressButton(entry: BtnEntry): void {
    ...
    setTimeout(() => {
      entry.mesh.position.z += .008; entry.mesh.scale.z = 1
      this.resetButtonProgress(entry)
    }, 140)
    entry.btn.onClick?.()
  }
...
export class WindowManager {
  ...
  // Hold state — без привязки к руке
    ...
    for (let hi = 0; hi < 2; hi++) {
      ...
      if (this.drag && this.drag.hi === hi) {
        if (g && g.grabStrength > .28) {
          ...
          this.drag.win.dragging = false; this.drag = null; this.cdDrag = 12
        }
      }
    }

    // ── CLOSE кнопка (three_finger на X-кнопку) ──────────────────────────────
    if (this.cdPress === 0) {
      for (let hi = 0; hi < 2; hi++) {
        const g = gestures[hi]; const ndc = fingerNDC[hi]
        ...
        for (const win of [...this.wins].reverse()) {
          ...
          if (win.hitCloseBtn(wp)) {
            win.onClose?.(); this.cdPress = 30; break
          }
        }
      }
    }

    // ── HOLD-TO-PRESS (THREE_FINGER на кнопках) ──────────────────────────────
    // Берём руку с наибольшим threeFingerStrength
    let bestG:   GestureResult | null = null
    let bestNDC: { ndcX: number; ndcY: number } | null = null
    for (let hi = 0; hi < 2; hi++) {
      const g = gestures[hi]
