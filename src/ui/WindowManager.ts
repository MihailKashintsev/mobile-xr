...
function btnTex(label:string,color:number,hov=false,progress=0):THREE.CanvasTexture{
  return canvasTex((ctx,W,H)=>{
    ...
    if(hov){
      ...
      ctx.fillStyle=g;ctx.beginPath();ctx.roundRect(4,4,W-8,H*.5,18);ctx.fill()
    }
    ctx.strokeStyle=hov?'rgba(120,160,255,.55)':'rgba(60,90,160,.22)'
    ctx.lineWidth=1.5;ctx.beginPath();ctx.roundRect(4,4,W-8,H-8,18);ctx.stroke()
...
function titleTex(t:string):THREE.CanvasTexture{
  return canvasTex((ctx,W,H)=>{
    ctx.fillStyle='rgba(210,225,255,.93)'
...
export class XRWindow{
  ...
  private build(title:string,content:WinContent):void{
    ...
    const txtW=this.closeable?W*.70:W*.84
    const tt=new THREE.Mesh(new THREE.PlaneGeometry(txtW,TH*.64),
      new THREE.MeshBasicMaterial({map:titleTex(title),transparent:true,depthWrite:false}))
    tt.position.set(this.closeable?-.042:0,H/2-TH/2,PD/2+.005);this.group.add(tt)
    ...
    for(let r=0;r<3;r++){
      const gl=new THREE.Mesh(new THREE.BoxGeometry(W*.28,.0016,PD+.006),
        new THREE.MeshBasicMaterial({color:0x243350,transparent:true,opacity:.60}))
      gl.position.set(0,-H/2+BRH/2+r*.016-.016,.002);this.group.add(gl)
    }

    this.glow=new THREE.Mesh(new THREE.BoxGeometry(W+.032,H+.032,PD+.032),
    ...
    this.border=new THREE.LineSegments(
      ...
      new THREE.LineBasicMaterial({color:C.border,transparent:true,opacity:.58}))
  ...
  private _buildButtons(btns:WinButton[]):void{
    ...
    const{W,H}=this
    const n=btns.length,cols=n<=2?n:n<=4?2:3,rows=Math.ceil(n/cols)
    const pad=.036,btnW=(W-pad*(cols+1))/cols
    const cTop=H/2-TH-pad,cBot=-H/2+BRH+pad
    const btnH=Math.min(.178,(cTop-cBot-pad*(rows-1))/rows)
    const totalH=btnH*rows+pad*(rows-1)
    const startY=(cTop+cBot)/2+totalH/2-btnH/2
    btns.forEach((btn,i)=>{
      const col=i%cols,row=Math.floor(i/cols)
      const x=-W/2+pad+col*(btnW+pad)+btnW/2
      const y=startY-row*(btnH+pad)
      const mesh=new THREE.Mesh(new THREE.BoxGeometry(btnW,btnH,BD),
        new THREE.MeshPhysicalMaterial({map:btnTex(btn.label,btn.color??0x1e293b),transparent:true,opacity:.96,roughness:.10}))
      mesh.position.set(x,y,PD/2+BD/2);this.group.add(mesh)
  ...
  hitCloseBtn(fw:THREE.Vector3):boolean{
  ...
  hitButtonByFinger(fw:THREE.Vector3):BtnEntry|null{
    ...
    for(const e of this.btnEntries){
      ...
      if(Math.abs(l.x)<p.width/2+.07&&Math.abs(l.y)<p.height/2+.07&&l.z<p.depth/2+.18&&l.z>-p.depth/2-.04)return e
    }
    return null
  }
  ...
  hitDragBar(wp:THREE.Vector3):boolean{
    ...
    return Math.abs(l.x)<this.W/2+.08&&l.y>-this.H/2-.06&&l.y<-this.H/2+BRH+.06&&l.z<.22&&l.z>-.06
  }
  ...
  hitButtonByRay(wp:THREE.Vector3):BtnEntry|null{
    ...
    for(const e of this.btnEntries){
      ...
      const p=(e.mesh.geometry as THREE.BoxGeometry).parameters
      if(Math.abs(l.x)<p.width/2+.04&&Math.abs(l.y)<p.height/2+.04&&Math.abs(l.z)<.18)return e
    }
    return null
  }
  ...
  resetButtonProgress(entry:BtnEntry):void{
    if(entry.holdProgress===0)return;entry.holdProgress=0
    const m=entry.mesh.material as THREE.MeshPhysicalMaterial
  ...
  setDragHighlight(on:boolean):void{
    ...
    ;(this.border.material as THREE.LineBasicMaterial).opacity=on?1:.58
  }
  setButtonHovered(entry:BtnEntry|null):void{
    for(const e of this.btnEntries){
      const hov=e===entry
      if(hov!==!!e.mesh.userData.wasHov){
        e.mesh.userData.wasHov=hov;if(e.holdProgress>0)continue
        const m=e.mesh.material as THREE.MeshPhysicalMaterial
        m.map?.dispose();m.map=btnTex(e.btn.label,e.btn.color??0x1e293b,hov,0);m.needsUpdate=true
        e.mesh.position.z+=hov?.004:-.004;e.mesh.scale.z=hov?1.35:1
      }
    }
  }
  ...
  set dragging(v:boolean){
    ...
    ;(this.border.material as THREE.LineBasicMaterial).opacity=v?1:.58
  }
  updateRenderOrder(camera:THREE.PerspectiveCamera):void{
    ...
    this.group.traverse((o:THREE.Object3D)=>{o.renderOrder=order})
  }
...
export class WindowManager{
  ...
  private cdDrag=0;private cdClose=0
  ...
  update(time:number,gestures:(GestureResult|null)[],fingerNDC:({ndcX:number;ndcY:number}|null)[],fingerWorld:(THREE.Vector3|null)[]):void{
    ...
    // Depth sort — окна зафиксированы в world space, quaternion не трогаем
    for(const w of this.wins) w.updateRenderOrder(this.camera)

    // ── CLOSE ──
    if(this.cdClose===0){
      for(let hi=0;hi<2;hi++){
        const g=gestures[hi];const fw=fingerWorld[hi]
        if(!fw||!g||g.threeFingerStrength<.55)continue
        for(const win of[...this.wins].reverse()){
          if(!win.group.visible)continue
          if(win.hitCloseBtn(fw)){win.onClose?.();this.cdClose=35;break}
        }
      }
    }
    ...
    for(let hi=0;hi<2;hi++){
      ...
      if(!this.drag&&grabbing&&this.cdDrag===0){
        for(const win of[...this.wins].reverse()){
          const wp=this.ndcToPlane(ndc.ndcX,ndc.ndcY,win.getWorldZ());if(!wp)continue
      ...
      if(this.drag&&this.drag.hi===hi){
        if(g&&g.grabStrength>.28){
          ...
          if(pt)this.drag.win.group.position.lerp(new THREE.Vector3(pt.x+this.drag.ox,pt.y+this.drag.oy,this.drag.planeZ),.36)
        }else{this.drag.win.dragging=false;this.drag=null;this.cdDrag=14}
      }
    }

    // ── HOLD-TO-PRESS ──
    let bestG:GestureResult|null=null,bestNDC:{ndcX:number;ndcY:number}|null=null,bestHi=0
    for(let hi=0;hi<2;hi++){
      const g=gestures[hi]
      if(g&&g.threeFingerStrength>(bestG?.threeFingerStrength??0)){bestG=g;bestNDC=fingerNDC[hi];bestHi=hi}
    }
    const pressing=!!(bestG&&bestG.threeFingerStrength>.52)
    let anyHit=false
    if(pressing&&bestNDC&&this.cdDrag===0){
      for(const win of this.wins){
        if(!win.group.visible||win.dragging)continue
        const wp=this.ndcToPlane(bestNDC.ndcX,bestNDC.ndcY,win.getWorldZ());if(!wp)continue
        ...
        if(hitEntry){
          ...
          if(this.holdState&&this.holdState.entry===hitEntry){
            ...
            if(progress>=1){win.pressButton(hitEntry);this.holdState=null;this.cdDrag=22}
          }else{
            if(this.holdState)this.holdState.win.resetButtonProgress(this.holdState.entry)
            this.holdState={entry:hitEntry,win,hi:bestHi,startTime:time}
